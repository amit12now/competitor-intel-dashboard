...
def add_record(**kw):
    rec = {
        ...
        "reactions": 0, "views": 0, "content_format": "",
    }
    rec.update(kw)
    RECORDS.append(rec)
...
def process_linkedin_file(fname, default_label):
    ...
    for p in posts:
        ...
        likes = metric_value(engagement.get("likes"))
        comments_count = metric_value(engagement.get("comments"))
        shares = metric_value(engagement.get("shares"))
        reactions = metric_value(engagement.get("reactions"))
        eng = likes + comments_count + shares
        content_format = "Video" if p.get("postVideo") else ("Image" if p.get("postImages") else "Text")
        ...
        add_record(
            ...
            author_type=author_type, engagement=eng, likes=likes,
            comments=comments_count, shares=shares, reactions=reactions,
    ...
    for d in data:
        ...
        if not competitor:
            ...
            if companies:
                ...
                continue
        ts = d.get("timestamp", "")
        likes = metric_value(d.get("likesCount"))
        comments_count = metric_value(d.get("commentsCount"))
        views = metric_value(d.get("videoViewCount") or d.get("videoPlayCount"))
        eng = likes + comments_count
        add_record(
            ...
            author_name=d.get("ownerFullName", owner), author_type="company",
            engagement=eng, likes=likes, comments=comments_count, views=views,
            content_format=d.get("type", ""),
        )
        kept += 1
...
def process_youtube():
    ...
    for d in data:
        ...
        date_iso = d.get("date", "")
        likes = metric_value(d.get("likes"))
        comments_count = metric_value(d.get("commentsCount"))
        views = metric_value(d.get("viewCount"))
        eng = likes + comments_count
        add_record(
            date=date_iso, month=month_label(date_iso), competitor=competitor,
            all_companies_mentioned=competitor,
            ...
            url=d.get("url", ""), author_name=ch, author_type="company",
            engagement=eng, likes=likes, comments=comments_count, views=views,
...
before_dedupe = len(df_final)
dedupe_key = ["month", "competitor", "channel", "mention_type", "url", "title"]
df_final = df_final.drop_duplicates(subset=dedupe_key, keep="first").reset_index(drop=True)
print(f"Dropped {before_dedupe - len(df_final)} duplicate scraped rows")
